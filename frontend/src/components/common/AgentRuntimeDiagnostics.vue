    ...
    <el-collapse v-model="activePanels" class="diagnostics-collapse">
      <el-collapse-item name="mcp">
        ...
        <div class="diagnostic-section" v-loading="mcpLoading">
          ...
          <div v-if="mcpToolGroups.length === 0" class="empty-box">暂无工具数据</div>
...
const mcpSummaryText = computed(() => {
  ...
  const { serverCount, toolCount } = countMcpToolGroups(mcpToolGroups.value)
  return `${mcpStatusText.value}，${serverCount} 个服务 / ${toolCount} 个工具`
})
