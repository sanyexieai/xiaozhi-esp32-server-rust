  ...
  <div class="config-page">
      ...
      <el-table-column label="测试结果" width="120" align="center">
        <template #default="scope">
          <template v-if="testResults[scope.row.config_id]">
            <el-tooltip
              :content="testResults[scope.row.config_id].ok ? formatTestResultTip(testResults[scope.row.config_id]) : formatTestMessage(testResults[scope.row.config_id])"
              placement="top"
            >
              <span :class="['test-result', testResults[scope.row.config_id].ok ? 'test-ok' : 'test-err']">
    ...
    <el-dialog v-model="dialogVisible" :title="editing ? '编辑配置' : '新增配置'" width="700px">
      ...
      <template #footer>
        ...
        <el-button type="warning" plain :loading="testingCurrent" @click="testCurrentConfig">测试</el-button>
...
const enabledItems = computed(() => items.value.filter((row) => row.enabled))
...
const buildKnowledgeSearchJson = () => {
...
const testConfig = async (row) => {
  ...
  try {
    const result = await testSingleConfig('knowledge_search', row.config_id)
    ...
    if (result.ok) {
      ...
      ElMessage.warning(`${row.name || row.config_id}：${formatTestMessage(result)}`)
    }
  } catch (err) {
  ...
  try {
    for (const row of list) {
      try {
        ...
        testResults.value = {
          ...testResults.value,
          [row.config_id]: { ok: false, message: err.response?.data?.error || '测试请求失败' }
        }
      }
    }
  } finally {
...
const testCurrentConfig = async () => {
  if (!String(form.config_id || '').trim()) {
    ElMessage.warning('请先填写配置 ID')
    return
  }
  ...
  try {
    const resultMap = await testWithData('knowledge_search', {
      [form.config_id]: {
        provider: form.provider,
        config: buildKnowledgeSearchJson()
      }
    })
    const item = resultMap?.[form.config_id]
    if (item?.ok) {
      ElMessage.success(formatTestMessage(item))
    } else {
      ElMessage.warning(formatTestMessage(item || { ok: false, message: '测试失败' }))
    }
  } catch (err) {
    ElMessage.error(err.response?.data?.error || '测试请求失败')
  } finally {
...
const submit = async () => {
  ...
  const payload = {
    ...
    json_data: JSON.stringify(buildKnowledgeSearchJson())
  }
  try {
    if (editing.value) {
