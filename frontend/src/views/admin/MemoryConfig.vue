      ...
      <el-table-column label="操作" width="260">
        <template #default="scope">
          ...
          <el-button
            ...
            @click="testConfig(scope.row, 'memory')"
          >
            测试
          </el-button>
          <el-button
...
const getEnabledConfigs = () => safeConfigs.value.filter((row) => row.enabled)
...
const testAllConfigs = async () => {
  ...
  try {
    for (const row of list) {
      try {
        const result = await testSingleConfig('memory', row.config_id)
        testResults.value = { ...testResults.value, [row.config_id]: result }
      } catch (err) {
        testResults.value = {
          ...testResults.value,
...
const testCurrentConfig = async () => {
  ...
  try {
    const result = await testWithData('memory', {
      [form.config_id]: {
        provider: form.provider,
        config: JSON.parse(generateConfig())
      }
    })
    const item = result?.[form.config_id]
    if (item?.ok) {
